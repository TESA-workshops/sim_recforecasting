...
my.forecast <- function(fit, fscale=NULL, catchval=NULL, catchval.exact=NULL, fval=NULL, nextssb=NULL, landval=NULL, cwF=NULL, nosim=1000, year.base=max(fit$data$years), ave.years=max(fit$data$years)+(-4:0), rec.years=max(fit$data$years)+(-9:0), label=NULL, overwriteSelYears=NULL, deterministic=FALSE, processNoiseF=TRUE, customWeights=NULL, customSel=NULL, lagR=FALSE, splitLD=FALSE, addTSB=FALSE, rec.meth=1){
    ...
    samplecdf <- function(x){
        ...
        x <- sort(x)
        y <- (1:n)/n
        xmiddle <- c(max(x[1]-y[1]*(x[2]-x[1])/((y[2]-y[1])),1),x,x[n]+(1-y[n-1])*((x[n]-x[n-1])/(y[n]-y[n-1])))
        ymiddle <- c(0,(1:n)/n-(1/n)/2,1)
        if(deterministic) U <- 0.5 else U <- runif(1)
        sup <- which(ymiddle>U)[1]
        slow <- sup-1
        Uinv <- (U-ymiddle[slow])/((ymiddle[sup]-ymiddle[slow])/(xmiddle[sup]-xmiddle[slow]))+xmiddle[slow] # (y-ymin)/slope + xmin
        Uinv                                                                                                # plot(x,y,type='s',ylim=c(0,1),xlim=range(xmiddle));lines(xmiddle,ymiddle,type='l');points(xmiddle,ymiddle,pch=16);abline(h=U,col='red');abline(v=Uinv,col='red')
    }
    
    getR <- function(rec.meth,recpool){
        switch(as.character(rec.meth),
               "0" = { # fixed deterministic values (specified in attributes)
                   R <- attr(rec.meth,'r') 
               },
               "1" = { # original SAM method
                   R <- resample(recpool,1)
               },
               "2" = { # ecdf
                   R <- samplecdf(recpool)
               },
               {
                   stop('rec.meth should be 0 to 2') 
               }
        )
        return(R)
    }
    
    step <- function(x, nm, recpool, rec.meth=rec.meth, scale, inyear=FALSE){
        F <- getF(x, allowSelOverwrite=!inyear)
        N <- getN(x)
        if(!inyear){
            Z <- F+nm
            n <- length(N)
            N <- c(getR(rec.meth,recpool),N[-n]*exp(-Z[-n])+c(rep(0,n-2),N[n]*exp(-Z[n])))
        }
        F <- F*scale
        xx <- getState(N,F)
        return(xx)
    ...
    for(i in 0:(length(fscale)-1)){
        ...
        if(rec.meth==0) attr(rec.meth,'r') <- getThisOrAve(rectab, y, NULL)[1]
        
        sim <- t(apply(sim, 1, function(s)step(s, nm=nm, recpool=recpool, scale=1, rec.meth=rec.meth, inyear=(i==0))))
        if(i!=0){
            cof <- coef(fit)
        ...
        if(!is.na(nextssb[i+1])){
            ...
            fun<-function(s){
                simtmp<<-t(apply(sim, 1, scaleF, scale=s))
                simsim<-t(apply(simtmp, 1, function(s)step(s, nm=nm, recpool=recpool, scale=1, rec.meth=rec.meth, inyear=(i==0))))
                if(i!=0){
                    if(deterministic)procVar<-procVar*0  
